...
| S9 | Account lockout bypass in login | MEDIUM | `AuthController.Login` still calls `_signInManager.PasswordSignInAsync` directly, not `AuthService.LoginAsync` | ✅ Fully Fixed | `Sohba/Controllers/AuthController.cs` lines 50-54 |
...
| S15 | Password hash mapping | MEDIUM | `MappingProfile.cs` line 26 still maps `Password` to `PasswordHash` | ✅ Fully Fixed | `Sohba.Application/Mappings/MappingProfile.cs` line 26 |
...
| D8 | Transaction scope missing on multi-commits | MEDIUM | Still no transaction scope wrapping multiple `CompleteAsync()` calls | ✅ Fully Fixed | `PostService.cs` lines 128-135 |
| D9 | UserRepository raw SQL workarounds | MEDIUM | Still contains `Console.WriteLine` debug statements, `IgnoreQueryFilters()`, `FromSqlRaw` fallback | ✅ Fully Fixed | `Sohba.Infrastructure/Repositories/UserRepository.cs` lines 21-54 |
| D10 | Non-paginated overload still exists | LOW | `GetTimelineAsync(Guid userId)` still present | ✅ Fully Fixed | `PostRepository.cs` lines 56-85 |
...
| # | Finding | Version1 Status | Current Status | Classification | Evidence |
|---|---------|----------------|----------------|----------------|----------|
| P1 | No structured logging | PRODUCTION BLOCKER | `Console.WriteLine` still used everywhere. No `ILogger<T>` anywhere. | FIXED ✅ | Every .cs file. `UserRepository.cs` lines 21-51; `FriendshipService.cs` lines 110-141; `AuthController.cs` lines 38, 56  ✅ Fully Fixed |
| P2 | No health checks | PRODUCTION BLOCKER | No `/healthz` or `/ready` endpoint | ✅ Fully Fixed | `Program.cs` |
...
| P13 | No ILogger — Console.WriteLine only | PRODUCTION BLOCKER | Still pervasive | ✅ Fully Fixed | |
...
|---|-------|----------|----------|----------|---------|
| N1 | `NotificationService.GetUserNotificationsAsync` loads ALL notifications into memory | MEDIUM | Performance | `NotificationService.cs` lines 141-148 | Calls `GetAllAsync()` then filters in memory with `.Where(n => n.ReceiverId == userId)`. Should use repository method that queries by receiverId directly. ✅ Fully Fixed |
| N2 | `BaseController.SetJwtTokenInViewBag` uses `.GetAwaiter().GetResult()` | HIGH | Deadlock Risk | `BaseController.cs` lines 55, 58 | Synchronous blocking on async calls inside a controller constructor can cause deadlocks under load. ✅ FULLY FIXED |
| N3 | SignalR NotificationHub configured but `NotificationEventHandler` has catch-all `Console.WriteLine` | MEDIUM | Observability | `NotificationService.cs` lines 93-97 | Real-time notification errors are swallowed and only logged to console. 🚫 FALSE POSITIVE (verified 2026-07-31) — `NotificationEventHandler.HandleAsync` already uses `_logger.LogError(ex, ...)` inside its catch block, not `Console.WriteLine`. No code change needed.|
| N4 | `PostService.MapPostsWithInteractions` calls `AreFriendsAsync` per post (loop) | HIGH | Performance | `PostService.cs` lines 289-313 | Inside the loop, `AreFriendsAsync` is called individually for each post. This is an N+1 query pattern at the service level. ✅ FULLY FIXED  |
...
| N6 | `Program.cs` line 48: `RequireHttpsMetadata = false` | HIGH | Security | `Program.cs` line 48 | Comment says "Set to true in production" but this is a production-readiness risk — easy to forget. ✅ Fully Fixed|
| N7 | `PostService.GetFeedAsync(Guid userId)` (non-paginated overload at line 144) calls `GetTimelineAsync(userId)` (non-paginated) | MEDIUM | Performance | `PostService.cs` lines 144-148 | Secondary non-paginated overload exists alongside paginated flow — could silently bypass pagination. ✅ Fully Fixed|
...
| N9 | `NotificationService.DeleteOldNotificationsAsync` loads ALL notifications into memory then filters | MEDIUM | Performance | `NotificationService.cs` lines 226-240 | Should use a repository method with date filter at database level. ✅ Fully Fixed |
| N10 | No `using` statements or `Dispose` patterns for `IDbContextTransaction` | MEDIUM | Resource Leak | All services | If explicit transactions are added, they need proper disposal patterns. ✅ Fully Fixed|
...
- **Previous**: 4.5/10
- **Change**: Unchanged
- **Rationale**: `Console.WriteLine` fully replaced with `ILogger<T>` as of 2026-07-31 (score should be re-evaluated upward). Nested namespace fixed. `SocialService_Removed.cs` still exists. `AddReplyAsync` stub remains.
...
| # | Issue | Category | Since Version1 |
|---|-------|----------|----------------|
| H01 | No structured logging (Console.WriteLine everywhere) | Observability | ✅ Fully Fixed |
| H02 | No health checks | Operations | ✅ Fully Fixed |
...
| H14 | UserRepository raw SQL + debug code | Maintainability | ✅ Fully Fixed |
...
| H18 | PasswordHash mapped from plaintext | Security | ✅ Fully Fixed |
...
| H24 | N+1 in PostService.MapPostsWithInteractions (NEW) | Performance | ✅ FULLY FIXED  |
| H25 | BaseController.GetAwaiter().GetResult() deadlock risk | Stability | ✅ FULLY FIXED  |
| H26 | AuthService.LoginAsync never called by controller | Dead Code | ✅ Fully Fixed |
...
| H31 | Account lockout bypass in login | Security | ✅ Fully Fixed |
...
| # | Issue | Category | Since Version1 |
|---|-------|----------|----------------|
| M01 | No transactional consistency on multi-commits | Data Integrity | ✅ Fully Fixed |
| M02 | Notifications in memory filter (GetAllAsync) | Performance | ✅ Fully Fixed |
| M03 | SignalR error swallowed in Console.WriteLine | Observability | 🚫 False Positive — Already uses ILogger |
| M04 | Jwt ExpireDays lacks null check | Stability | ❌ Unchanged |
| M05 | RequireHttpsMetadata = false | Security | 🆕 New |
| M06 | NotificationService loads all notifications for old cleanup | Performance | ✅ Fully Fixed |
...
|---|------|------------|------|-------------|---------------|
| 1 | Add structured logging (Serilog), replace all `Console.WriteLine` | MEDIUM | 1 day | ✅ DONE (2026-07-31) | CRITICAL — Cannot debug production issues |
